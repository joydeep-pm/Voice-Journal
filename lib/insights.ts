import type { Entry, WeeklyTheme } from '@/lib/types';
  ...
  return [...grouped.entries()]
    ...
    .map(([week, weekEntries]) => {
      ...
      for (const entry of weekEntries) {
        const source = [entry.title ?? '', entry.summary ?? '', entry.transcript ?? ''].join(' ');
        for (const word of normalizeWords(source)) {
      ...
      return {
        week,
        theme: topWords.length ? topWords.join(', ') : 'General reflections',
